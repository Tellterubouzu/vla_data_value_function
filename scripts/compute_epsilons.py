...
from src.utils.metrics import logits_to_continuous
from src.utils.seed import set_seed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute task-wise epsilon from predicted value")
...
def autocast_context(device: torch.device, amp_enabled: bool, amp_dtype):
    if amp_enabled:
...
def main() -> None:
    ...
    if full_state_path.exists():
        model.load_state_dict(torch.load(full_state_path, map_location="cpu"), strict=True)
    elif trainable_state_path.exists():
        model.load_trainable_state_dict(torch.load(trainable_state_path, map_location="cpu"))
    else:
    ...
    dataset = AiroaMomaValueFrameDataset(
    ...
    pred_by_task: Dict[str, List[float]] = {}

    with torch.no_grad():
        for batch in tqdm(loader, desc="compute_epsilons"):
            tasks = batch["task"]
            ...
            v_hat_list = v_hat.detach().cpu().tolist()

            for task, value in zip(tasks, v_hat_list):
                pred_by_task.setdefault(task, []).append(float(value))

    if not pred_by_task:
        raise RuntimeError("No predictions collected; dataset may be empty")

    epsilons = {
        task: float(np.percentile(np.array(values, dtype=np.float32), 30))
        for task, values in sorted(pred_by_task.items(), key=lambda x: x[0])
    }

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", encoding="utf-8") as f:
        json.dump(epsilons, f, indent=2, ensure_ascii=False)

    print(f"saved epsilons: {out_path}")
    print(f"tasks={len(epsilons)}")
